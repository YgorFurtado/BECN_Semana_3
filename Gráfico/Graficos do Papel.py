...
print("O Coeficiente angular medido no paquimetro é:" + str(coef_ang_paq))
print("O Coeficiente angular medido na régua é:" + str(coef_ang_paq))

#Cria os gráficos Diâmetro x Massa:
plt.subplot(221)
plt.errorbar(diam_r,massa,xerr = erro_r,fmt = "D:",label="Regua")
plt.errorbar(diam_p,massa,xerr = erro_p,fmt = "s--",label="Paquimetro")
plt.grid()
...
plt.subplot(222)
plt.plot(log_regua,log_massa,"s--",label="Regua",)
plt.plot(log_paquimetro,log_massa,"D:",label="Paquimetro",)
plt.grid()
...
plt.subplot(212)
plt.axline((1,y_reg),slope=coef_ang_reg,ls = (5, (10, 3)),label = "Regua")
plt.axline((1,y_paq),slope=coef_ang_paq,ls = (0, (3, 1, 1, 1, 1, 1)),label="Paquimetro",color = "orange")
plt.grid()
plt.xlabel("Log(Diametro)")                         
plt.ylabel("Log(Massa)")
plt.title("Log x Log Ajeitado")
plt.legend()
plt.xlim(1.51,3.80)
plt.ylim(-3.9,2.1)

#Mostra os gráficos
plt.suptitle("Gráficos do Papel Comum")
plt.show()